...
- [x] CONTEXT-02 允许用户逐条确认模型归纳的事实，未确认内容不得当作偏好

## P4：价格证据与入手时机

- [x] PRICE-00 本地保存真实价格快照、目标价和提醒状态；支持手动检查与启动时限频检查
- [x] PRICE-00B 目标价首次命中提醒、重复提醒抑制、暂停/删除，以及备份导入导出
- [x] PRICE-01 建立可替换的价格数据提供层，记录来源、采集时间和商品匹配置信度
- [x] PRICE-02 展示当前价、可验证参考价与近期低价；无可靠数据时明确说“不知道”
- [x] PRICE-03 将价格时机作为建议的一项证据，不覆盖需求、预算和已有物品判断

## P5：偏好画像与趣味分享

## P6：价格历史可视化与促销套路检测

- [x] PRICE-VIZ-01 价格 sparkline（CustomPainter，无外部依赖）
- [x] PRICE-VIZ-02 "先涨后降"促销套路检测与警告

## P7：推荐闭环精修

- [x] CONTEXT-03 分析前明确选择真正相关的现有物品，粗分类仅用于自动预选
- [x] CONTEXT-04 已提供现有物品但关系判断缺失时，本地守卫禁止直接建议购买
- [x] TAXONOMY-01 为耳机、键鼠、手机、电脑等建立可迁移的细分类或商品类型，减少粗分类噪声
- [x] PRICE-04 接入用户可选的外部历史价格提供器；未配置时继续只称“本机记录低价”
- [x] PROFILE-04 为消费人格分享卡补真实分享面板/系统分享入口，并保留保存图片降级路径

- [x] PREF-01 只从已确认购买、使用频率、满意度和后悔反馈生成候选偏好
- [x] PREF-02 推荐时只使用用户确认过的偏好，并展示引用来源和反例
- [x] PROFILE-01 生成可编辑的“消费人格”趣味结果与分享卡
- [x] PROFILE-02 分享卡默认不含商品明细、金额、订单、模型配置等隐私数据
- [x] PROFILE-03 明确标注为娱乐性总结，不包装成心理测评或科学结论
