...
应用只保存价格提供者实际返回的价格，不会生成或猜测价格。每条记录都带有来源、采集时间和商品匹配置信度；匹配置信度低于 80% 时只保留诊断记录，不会更新可信当前价或触发通知。当前默认提供者是 JustOneAPI，以后可以替换为其他合法数据源，而不需要改动提醒逻辑。

第一次使用时没有全网历史曲线，只能展示从你开始监测后的本地记录。应用启动时会检查超过 6 小时未更新的商品；也可以随时手动检查。系统可能限制后台运行，因此当前版本不能承诺在价格变化的瞬间提醒。

### 7. 告诉应用你已经有什么

1. 打开“习惯”页，在“我的物品”旁点击“添加一件”。
...
如果有一批历史购买记录，可以点击“导入购买清单”。每行按“名称 | 分类 | 当前状态 | 当时价格 | 日期 | 备注”填写；价格、日期和备注可留空。应用会先展示预览，格式错误和同名同分类的重复项不会直接写入。未填写状态的记录只会标记为“买过，现状不确定”，不会被当作当前仍然拥有。

也可以点击“订单截图”，选择淘宝、京东等平台的历史订单截图。图片仍使用 Android ML Kit 或 Apple Vision 在设备本地识别，不发送给模型。应用会排除“实付款”“合计”等订单总额，并把疑似单品整理成草稿；写入前可以逐项修改名称、分类、状态、价格、日期和备注。OCR 无法可靠配对的截图不会自动生成历史记录。

模型有时会根据本次理由、历史或已有物品提出少量“可能是你的习惯”。这些内容默认不采用，并会同时展示依据。只有你在结果页勾选确认，或之后在“习惯”页确认，它才会进入下一次分析；候选和已忽略内容不会发送给模型。
