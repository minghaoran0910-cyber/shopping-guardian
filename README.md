...
第一次使用时没有全网历史曲线，只能展示从你开始监测后的本地记录。应用启动或回到前台时会检查超过 6 小时未更新的商品；也可以随时手动检查。可信报价首次进入目标价区间时，系统通知会明确告诉你“达到目标价，可以考虑下单”。系统可能限制后台运行，因此当前版本不能承诺在价格变化的瞬间提醒。

“稍后再看”里的价格证据有意采用保守口径：

- 当前价只显示 24 小时内、商品匹配置信度不低于 80% 的最新实测报价。
- 参考价指上一条可信实测报价，不是平台划线价或商家宣称的原价。
- 近期低价只表示本机开始记录后的 30 天低价，至少需要两次可信采样且间隔 6 小时。
- 每个可用价格都会显示数据来源和采集时间；证据不足时直接显示“不知道”。

分析单件商品时，应用会查找同平台、同商品的本机监测记录，并在发送前展示当前价、上次可信价和本机 30 天低价。价格只用于判断“现在买还是再等等”：真实需求、预算、消费规则和已有同类物品始终优先。即使当前价接近本机低价，也不会把超预算、命中等待规则、重复购买或需求不明确的商品改成“可以买”。购物车多件商品没有唯一可归属的价格记录，因此会明确显示证据不足，建议逐件分析。

如果“我的物品”里已经记录了当前耳机、键盘等设备，分析前可以明确勾选本次真正相关的物品。分类只负责自动预选；同属“数码”但无关的物品可以排除。模型必须说明新商品是在替代、补充还是重复，关系不清楚时应用不会直接放行购买建议。
...
应用自动整理的消费偏好采用更严格的口径：至少需要三条同分类或同标签的已确认购买，而且每条都必须有使用频率、满意度或后悔反馈。等待过、放弃过、只分析过，或者买了但还没填写购后体验的记录，都不会进入候选偏好，也不会作为反例凑数量。

“习惯”页可生成可编辑的娱乐性消费人格卡，并直接打开系统分享面板；也可只保存图片。分享内容默认不含商品明细、金额、订单和模型配置。

分析时只会发送用户已经确认、且仍能找到有效支持记录的偏好。发送前核对页和建议结果会同时列出支持依据与相反记录；如果原始决策已经删除，相关证据会同步失效，失去全部支持依据的偏好不会继续发送。模型只能解释这些本地证据，不能自行补写偏好来源。
...
淘宝或京东单品完成分析后，可以打开“到目标价时提醒我”并填写目标价。应用会用用户自己的 JustOneAPI 查询真实报价，把可信报价和本机历史写入本地数据库；达到目标价时发送“可以考虑下单”的系统通知。

监测记录在“稍后再看”中管理，可以立即检查、修改目标价、暂停、恢复或删除。创建和修改目标价后会立即查一次；之后会在应用启动或回到前台时限频检查。个人本地应用不保证被系统关闭后仍能 24 小时常驻，通知时间受 Android、iOS 和桌面系统后台策略影响。

## 消费人格趣味卡

“习惯”页可以根据至少三条有来源的已确认偏好生成消费人格；证据不足时改用四道完全本地的趣味小题，不调用模型。标题、三个特点和一句提醒都可以编辑并保存为 PNG。

分享卡只包含上述趣味结果、免责声明和项目地址，不包含商品、价格、订单、API Key 或模型配置。它是娱乐性消费总结，不是心理测评或科学结论。

消费人格会进入业务数据 JSON 备份。导入时，“合并”保留本机已经编辑的人格；“覆盖本机”使用新版备份中的人格，新版备份明确为空时会删除本机人格。旧版备份不了解该字段，因此覆盖导入旧备份时仍会保留本机人格。
...
- 决策、预算、规则、价格监测和消费人格存入本机 SQLite 数据库，设置存入本机偏好；数据库与 JSON 导出未额外加密，请保护好设备和备份文件
